...
class ProbabilityStatisticsToolkit:
    ...
    def cusum_filter(self, returns: pd.Series, threshold: float) -> list:
        """Symmetric CUSUM filter (event-based sampling) — Lopez de Prado,
        *Advances in Financial Machine Learning*, ch. 2.5.2.1.

        Provenance: generated by Agent 8 (backend/agents/codegen/), attempt 1
        of 1, groq/openai/gpt-oss-120b, 2026-08-19 — verified against an
        independently hand-traced reference case before being accepted (see
        backend/agents/codegen/__main__.py's VERIFICATION_SCRIPT for the
        exact trace), not hand-written. Closes the AFML entry in
        docs/research/RESOURCES-LOG.md that a prior research-agent dispatch
        failed to process.

        Flags a point whenever cumulative drift since the last flagged event
        exceeds `threshold` in either direction, then resets both running
        sums — the standard way to convert a fixed-interval return series
        into event-driven samples for downstream labeling/backtesting.
        """
        if threshold <= 0:

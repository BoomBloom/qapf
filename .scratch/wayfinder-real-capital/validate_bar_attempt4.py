"""Ticket 07 — attempt 4 of 5.

Structurally different from attempts 1-3, per the explicit lesson attempt 3
surfaced: DSR trial-count is now the binding constraint, so this attempt
picks the candidate with the FEWEST free parameters, not the highest
expected raw improvement. docs/research/viable-alpha-families.md's #4
(risk-based allocation) argues for "zero alpha parameters and a near-honest
DSR trial count of ~1-4" by dropping return forecasting entirely.

APPROXIMATION, STATED PLAINLY: a true minimum-variance portfolio needs a
weight-level optimizer (Qlib's PortfolioOptimizer, already used in Agent 2's
LIVE allocation path), but WalkForwardBacktester's historical engine is
built around Qlib's TopkDropoutStrategy, which consumes a per-name RANKING
signal, not target weights directly -- a real architecture mismatch between
the live and backtest paths, not something to paper over. Rather than
rebuild the backtest engine's execution strategy under time pressure, this
attempt tests the honest, well-documented simplification: naive risk parity
/ inverse-volatility tilt -- rank purely by the (now Yang-Zhang-based)
low_volatility factor, zero weight on momentum/reversal/volume. This is a
real, literature-supported risk-based method on its own (Clarke, de Silva &
Thorley and others), not a fake stand-in for minimum-variance, but it is
NOT the same thing as full covariance-based GMV -- recorded honestly, not
blurred.

Monkeypatches agents.alpha.combiner.REGIME_FACTOR_WEIGHTS (looked up fresh
per call, same mechanism the Yang-Zhang diagnostic used for FACTOR_FUNCTIONS)
rather than committing this weight configuration as production code before
knowing whether it helps -- ticket 06 already established that regime-
weight changes are a real, considered decision, not something to land
speculatively.
"""
...
def main():
    ...
    for regime in combiner_module.REGIME_FACTOR_WEIGHTS:
        combiner_module.REGIME_FACTOR_WEIGHTS[regime] = dict(RISK_BASED_WEIGHTS)

    try:
        print(f"\n=== Running walk-forward backtest: pure low-vol tilt (attempt 4/5) ===")
    ...
    print(f"\n=== Results: {TEST_START} to {TEST_END} (pure low-vol tilt + vol-managed) ===\n")
    ...
    print(f"\n{'CLEARS THE BAR' if overall else 'DOES NOT CLEAR THE BAR'} (attempt 4 of 5)")

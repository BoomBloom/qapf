...
## Batch 6 — Stocklake MCP connector (2026-08-20)

| Resource | Status | Verdict |
|---|---|---|
| [stocklake.dev](https://api.stocklake.dev/mcp) (MCP connector + API key) | ❌ for QAPF's open tickets · 🔍 possible future use | **Verified by direct API call, not marketing copy** — enumerated all 17 MCP tools over JSON-RPC and made a real `get_stock_history` request with the operator's key. **Decisive finding: historical daily OHLCV is hard-capped at 365 bars** (requested 3,000, received exactly 365; earliest bar 2025-03-07). QAPF's ticket 02 validation window is **2008-2017 — ten years**, so this cannot run the validation backtest at all. It also has **no intraday data whatsoever** (ticket 11 needs minute bars) and no delisted-security coverage (ticket 13's actual gap). **It therefore solves none of the three open data problems**, despite being pitched as stock-market data for AI agents. Second, deeper concern: most of its differentiated tools are *AI-synthesized judgments* (`get_signals`, `get_market_assessment`, `get_sector_intelligence`, `get_insider_activity` → "combined signal BULLISH/BEARISH/NEUTRAL"), which is the opposite of this project's stated design — Agent 6 already produces a **deterministic, reproducible, free** growth×inflation regime from FRED, and CLAUDE.md's rules favor verifiable computation over unverifiable LLM opinion inside the decision path. Adopting an opaque third-party AI signal would also silently break Agent 9's look-ahead guarantees (no way to prove a vendor's "AI assessment" was computed point-in-time). Operator is unidentified on the site. Pricing: free 200 calls/day, Pro $20/mo, 5,000/day. **Where it could legitimately fit later:** Agent 3 (Research) news ingestion, or a live paper-trading dashboard widget — forward-looking, non-backtested surfaces where its 365-day/AI-summary nature isn't disqualifying. Key stored in `.env` as `STOCKLAKE_API_KEY`; nothing in `backend/` imports it. |

## Batch 7 — pwb-alphaevolve (2026-08-20)

| Resource | Status | Verdict |
|---|---|---|
| [BoomBloom/pwb-alphaevolve](https://github.com/BoomBloom/pwb-alphaevolve) | ❌ not adopted, 💡 idea worth keeping | **Verified real by cloning and reading source, not the README** — 2,262 lines, genuine tests (520 lines across test_engine/test_controller/test_prompt_ga), real OpenAI client, real Backtrader-based evaluator, SQLite experiment store. 0 stars/0 forks (brand new, unvetted by anyone else), but functional. LLM-guided evolutionary strategy discovery inspired by DeepMind's AlphaEvolve: generate strategy code, backtest, keep winners, mutate, repeat. **Not adopted for three concrete reasons**: (1) requires OpenAI's o3 specifically, non-optional — an evolutionary loop makes many LLM calls by design, directly contradicting this project's Groq/Anthropic cost-discipline decision; (2) different backtest engine (Backtrader) and data source (Papers-With-Backtest), none of QAPF's hard-won Qlib/point-in-time verification work transfers; (3) most substantive — an automated code-mutate-and-keep-backtest-winners loop is exactly the overfitting machine `docs/research/viable-alpha-families.md`'s DSR critique warned about (every generation is another trial), and the README shows no DSR/PBO/overfitting correction. **The concept is worth remembering as a future direction for Agent 8** if ever built in-house with the same rigor (Qlib, honest trial-count tracking, Groq/Anthropic tiers) rather than adopted as-is. |

## Batch 8 — paperswithbacktest.com (2026-08-20)

| Resource | Status | Verdict |
|---|---|---|
| [paperswithbacktest.com](https://paperswithbacktest.com/) / `pwb-toolbox` (the data source behind Batch 7's alphaevolve repo) | ❌ for tickets 11 and 13 | **Verified by downloading and querying the actual data, not the marketing claims.** Real, public dataset (`paperswithbacktest/Stocks-Daily-Price` on HuggingFace, 6.5M+ rows/shard x4 shards, no paid key needed for the public HF mirror). Marketing claims "survivorship-adjusted" coverage; tested directly against the same four 2008 names ticket 13 already verified in the free PIT fix: **LEH, FNM, FRE are absent entirely.** "WB" IS present — but its history starts 2014-04-17, which is exactly when **Weibo Corporation** IPO'd under that ticker, not the original Wachovia (acquired by Wells Fargo Dec 2008). The dataset has a *different* company that later reused the symbol — a ticker-collision trap, not real delisted-security coverage. "Survivorship-adjusted" in their marketing almost certainly means point-in-time index membership (which ticket 13 already solved for free), not actual delisted prices. Schema is daily OHLCV only (`symbol, date, open, high, low, close, volume, adj_close`) — no intraday, so it wouldn't help ticket 11 either. **Solves neither remaining data gap.** Pricing (Backtester tier, $50/mo or $450/yr) not worth pursuing on this evidence. |
